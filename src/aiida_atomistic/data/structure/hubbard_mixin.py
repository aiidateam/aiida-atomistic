...
class HubbardGetterMixin:
    def get_hubbard_list(
        self,
    ) -> List[Tuple[int, str, int, str, float, Tuple[int, int, int], str]]:
...
class HubbardSetterMixin:
    ...
    def set_hubbard_from_list(
        ...
        self.properties.hubbard = Hubbard(
            parameters=parameters, projectors=projectors, formulation=formulation
        )
        return
    ...
    def append_hubbard_parameter(
        ...
        if translation is None:
            _, translation = sites[atom_index].distance_and_image(
                sites[neighbour_index]
            )
    ...
    def _get_symbol_indices(self, symbol: str) -> List[int]:
        ...
        matching_kinds = [
            kind
            for kind, symbols in zip(
                self.properties.kind_names, self.properties.symbols
            )
            if symbol == symbols
        ]
