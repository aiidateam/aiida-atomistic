...
class HubbardUtils:
    ...
    def __init__(self, hubbard_structure: Union[StructureData, StructureBuilder]):
        """Set a the `HubbardStructureData` to manipulate."""
        if isinstance(hubbard_structure, StructureData) or isinstance(
            hubbard_structure, StructureBuilder
        ):
            self._hubbard_structure = hubbard_structure
        else:
            raise ValueError(
                f"input is not of type `StructureData`, `StructureBuilder`, but `{type(hubbard_structure)}`"
            )
    ...
    def get_hubbard_card(self) -> str:
        ...
        for param in hubbard_parameters:
            ...
            index_j = (
                get_supercell_atomic_index(
                    param.neighbour_index, natoms, param.translation
                )
                + 1
            )
            ...
            if hubbard.formulation not in ["dudarev", "liechtenstein"]:
                raise ValueError(
                    f"Hubbard formulation {hubbard.formulation} is not implemented."
                )
    ...
    def parse_hubbard_dat(self, filepath: Union[str, os.PathLike]):
        ...
        if not isinstance(self.hubbard_structure, StructureBuilder):
            raise ValueError(
                "the input is not of type `StructureBuilder`, so it cannot be modified. \
        ...
        for data in hubbard_data:
            if data[0] == "U":
                ...
                index = int(
                    self.hubbard_structure._get_one_kind_index(manifold.pop(0))[0]
    ...
    def reorder_atoms(self):
        ...
        index_map = {
            ...
            if site.kind_name in hubbard_kinds
        }
        ...
        for parameter in parameters:
            ...
            new_parameter[2] = site_map.index(
                index_map[parameter[2]]
            )  # neighbour index
    ...
    def get_hubbard_for_supercell(
        self, supercell: StructureBuilder, thr: float = 1e-3, mutable=True
    ) -> Union[StructureData, StructureBuilder]:
        ...
        for param in hubbard.parameters:
            ...
            for sc_i_index, sc_i_translation in zip(sc_i_indices, sc_i_translations):
                j_position = sc_j_position + np.dot(
                    sc_i_translation - uc_j_translation + param.translation, uc_cell
                )
                local_site = sc_pymat.get_sites_in_sphere(pt=j_position, r=thr)[
                    0
                ...
                sc_hubbard_parameter = [
                    ...
                    param.value,
                    np.array(
                        local_site.image, dtype=np.int64
                    ).tolist(),  # otherwise the class validator complains
                    param.hubbard_type,
                ]
...
def get_supercell_atomic_index(
    index: int, num_sites: int, translation: List[Tuple[int, int, int]]
) -> int:
...
def get_index_and_translation(
    index: int, num_sites: int
...
def is_intersite_hubbard(hubbard: Hubbard) -> bool:
    ...
    couples = [
        (param.atom_index != param.neighbour_index) for param in hubbard.parameters
    ]
    return any(couples)
